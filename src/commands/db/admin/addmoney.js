...
module.exports.run = async (bot, message, args) => {
    ...
    if(!args[1]) return message.channel.send(mensagem.digite_coins)
    ...
    db.add(`coins_${membro.id}`, args[1])

    const logsEmbed = new MessageEmbed()
    .setColor("ORANGE")
    .setAuthor(`${membro.username}#${membro.discriminator}`, membro.displayAvatarURL())
    .setTitle("LOGS - Coins adicionados")
    .setDescription(`${message.author.username} adicionou ${args[1]} coins para o usuário ${membro.user.username}`)
    .setTimestamp()
...
module.exports.help = {
    name: "addmoney",
    aliases: ["adicionarmoney", "addMoney"]
}