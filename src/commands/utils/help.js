...
module.exports.run = async (bot, message, args) => {
    ...
    .addComponents(
        new MessageButton()
        .setStyle('SUCCESS')
        .setCustomId(botoes.ajudaCommand.home)
        .setEmoji("\🏡")
    )
    ...
    .addComponents(
        new MessageButton()
        .setStyle('SUCCESS')
        .setCustomId(botoes.ajudaCommand.id)
        .setEmoji("\🍃")
    )
    ...
    .addComponents(
        new MessageButton()
        .setStyle('SUCCESS')
        .setCustomId(botoes.ajudaCommand.id2)
        .setEmoji("\👮")
    )
    ...
    .addComponents(
        new MessageButton()
        .setStyle('SUCCESS')
        .setCustomId(botoes.ajudaCommand.id3)
        .setEmoji("\💰")
    )
    ...
    .addComponents(
        new MessageButton()
        .setStyle('SUCCESS')
        .setCustomId(botoes.ajudaCommand.id4)
        .setEmoji("\⚙️")
    )
    ...
    .addComponents(
        new MessageButton()
        .setStyle('SUCCESS')
        .setCustomId(botoes.ajudaCommand.id5)
        .setEmoji("\🎮")
    )
    .addComponents(
        new MessageButton()
        .setStyle('SUCCESS')
        .setCustomId(botoes.ajudaCommand.id6)
        .setEmoji("<a:minecraft:780777822435672084>")
    )
    ...
    collector3.on(`collect`, async m => {
        if(m.customId === 'EcoButton') {
            ...
            .setDescription("`\`\`\ daily | depall | rep | roubar | saldo | work\`\`\`")
            await m.deferUpdate()
    ...
    collector4.on(`collect`, async m => {
        if(m.customId === 'ConfigButton') {
            ...
            .setDescription("`\`\`\ setbooster | setleave | setlogs | setpunição | setwelcome\`\`\`")
            await m.deferUpdate()
    ...
    collector6.on(`collect`, async m => {
        if(m.customId === 'MinecraftButton') {
            ...
            .setColor("PURPLE")
            .setTitle("Categoria Jogos")
            .setDescription("`\`\`\ mcserver | mcskin\`\`\`")
            await m.deferUpdate()
